...
int nAnonymizeEarnzCoinAmount = 1000;
...
#else
    const char* pszModule = "EarnzCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EarnzCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EarnzCoin
    // Mac: ~/Library/Application Support/EarnzCoin
    // Unix: ~/.EarnzCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EarnzCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "EarnzCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "EarnzCoind.pid"));
