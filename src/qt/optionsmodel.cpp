...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeEarnzCoinAmount"))
        settings.setValue("nAnonymizeEarnzCoinAmount", 1000);
    nAnonymizeEarnzCoinAmount = settings.value("nAnonymizeEarnzCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeEarnzCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeEarnzCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeEarnzCoinAmount:
            return QVariant(nAnonymizeEarnzCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeEarnzCoinAmount:
            nAnonymizeEarnzCoinAmount = value.toInt();
            settings.setValue("nAnonymizeEarnzCoinAmount", nAnonymizeEarnzCoinAmount);
            emit AnonymizeEarnzCoinAmountChanged(nAnonymizeEarnzCoinAmount);
            break;
        default:
            break;
        }
