...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeEarnzCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeEarnzCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeEarnzCoinAmount = strAnonymizeEarnzCoinAmount.remove(strAnonymizeEarnzCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEarnzCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeEarnzCoinAmount*COIN) nMaxToAnonymize = nAnonymizeEarnzCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeEarnzCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeEarnzCoinAmount));
        strAnonymizeEarnzCoinAmount = strAnonymizeEarnzCoinAmount.remove(strAnonymizeEarnzCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEarnzCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeEarnzCoinAmount == 0){
