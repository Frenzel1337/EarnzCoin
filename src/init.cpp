...
void Shutdown()
{
    ...
    RenameThread("EarnzCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: EarnzCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: EarnzCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeEarnzCoinamount=<n> " + _("Keep N EarnzCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. EarnzCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. EarnzCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("EarnzCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "EarnzCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of EarnzCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart EarnzCoin to complete") << "\n";
    ...
    nAnonymizeEarnzCoinAmount = GetArg("-anonymizeEarnzCoinamount", 0);
    if(nAnonymizeEarnzCoinAmount > 999999) nAnonymizeEarnzCoinAmount = 999999;
    if(nAnonymizeEarnzCoinAmount < 2) nAnonymizeEarnzCoinAmount = 2;
    ...
    LogPrintf("Anonymize EarnzCoin Amount %d\n", nAnonymizeEarnzCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1EarnzCoin+1000 == (.1EarnzCoin+100)*10
       10EarnzCoin+10000 == (1EarnzCoin+1000)*10
    */
